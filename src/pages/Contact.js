        ...
        <div className="py-10 px-8">
          <div className="w-full lg:w-1/2 flex flex-col space-y-3 pb-8  overflow-hidden">
            ...
            <p className="text-[#141414] text-[36px] sm:text-[48px] md:text-[56px] lg:text-[74px] font-bold flex gap-2 overflow-hidden flex-wrap ">
              <p ref={cus2Ref}>Become</p>
              <p ref={cus3Ref}>a</p>
              <p ref={cus4Ref}>Client</p>
            </p>

            <p ref={cus7Ref} className="text-[#4A4A4A] text-[16px] lg:text-[20px] font-medium overflow-hidden">
              Discover how we can make a difference for your business. Fill out the form below to get started, and one of our representatives will be in touch with you shortly. Let's collaborate and achieve great things together.{' '}
            </p>
          </div>
            ...
            {!submitted ? (
              <form onSubmit={handleSubmit} className="flex flex-col lg:flex-row items-center space-y-3 lg:space-y-0 lg:space-x-3 ">
                <div className="w-full lg:w-1/4">
                  <input type="text" required id="name" value={name} onChange={(e) => setName(e.target.value)} className="flex w-full p-[16px] items-center self-stretch rounded-[50px] border-[1px] focus:outline-none focus:shadow-outline" placeholder="Name" />
                </div>

                <div className="w-full lg:w-1/4">
                  <input type="email" required id="email" value={email} onChange={(e) => setEmail(e.target.value)} className="flex w-full p-[16px] items-center self-stretch rounded-[50px] border-[1px] focus:outline-none focus:shadow-outline" placeholder="Email@company.com" />
                </div>
                {isIndia && (
                  <div className="w-full lg:w-1/4">
                    <input type="tel" required id="phone" value={phone} onChange={(e) => setPhone(e.target.value)} className="flex w-full p-[16px] items-center self-stretch rounded-[50px] border-[1px] focus:outline-none focus:shadow-outline" placeholder="+91 XXXXX-XXXXX"  />
                  </div>
                )}
                <button onMouseEnter={handleMouseEnterContact} onMouseLeave={handleMouseLeaveContact} type="submit" className="flex py-3 px-8 gap-[8px] rounded-full bg-black text-[16px] font-semibold leading-[18px] text-white hover:shadow-xl border-[1px] border-black hover:bg-white hover:border-[1px] hover:border-gray-300 hover:text-black">
                  Get in Touch
                </button>
              </form>
            ) : (
